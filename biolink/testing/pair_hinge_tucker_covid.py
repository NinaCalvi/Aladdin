...
space = {'model': hp.choice('model', ['tucker']), \
        'data': hp.choice('data', ['covid']), \
        'loss': hp.choice('loss', ['pair_hinge']), \
        ...
        'reg-weight': hp.loguniform('reg-weight', np.log(1.0e-10), np.log(1.0e-01)), \
        'embedding-size': scope.int(hp.quniform('embedding-size', 50, 200,50)), \
        'rel-emb-size': scope.int(hp.quniform('real-emb-size', 50, 200,50)), \
        'loss-margin': hp.choice('loss-margin', [1,2,3,4,5]), \
        'quiet': hp.choice('quiet', [True]), \
